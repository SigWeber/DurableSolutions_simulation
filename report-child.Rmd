...
combined_results <- 
  ...
  pivot_wider(names_from = "metric", values_from = "Durable_Solutions")
...
metric_pairs <-
  ...
  mutate(kappa = map2_dbl(x, y, 
                          ~furrr::future_map2_dbl(combined_results[[.x]], combined_results[[.y]],
                                    function(m1, m2) { 
                                      if (is_empty(m1) | is_empty(m2)) return(NA_real_)
                                      left_join(m1, m2, by = "HHID") %>% 
                                        mutate(across(exited.x:exited.y, as_factor)) %>% 
                                        yardstick::kap(exited.x, exited.y) %>% 
                                        pluck(".estimate")}) %>%
                            mean(na.rm = TRUE)))

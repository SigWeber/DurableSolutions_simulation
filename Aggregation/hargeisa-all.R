...
run_simulation <- function(data, metric, ..., combinations = NULL) {
  ...
  combinations <- combinations %>% slice_sample(n = 1000, replace=TRUE)
...
simulation_hargeisa <- bind_rows(
  ...
  simulate_ecdf(hargeisa) |> mutate(metric = "6: Empirical cumulative distribution"))%>% 
  select(-Durable_Solutions)
# write_csv(simulation_hargeisa, "Aggregation/hargeisa_all.csv")
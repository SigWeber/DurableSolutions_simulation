...
DS_Option5b_nigeria <- targets::tar_read("DS_Option5b_nigeria") %>% select(-Durable_Solutions)
...
DS_Option5b_hargeisa <- targets::tar_read("DS_Option5b_hargeisa") %>% select(-Durable_Solutions)
...
DS_Option4b_sudan <- targets::tar_read("DS_Option4b_sudan") %>% select(-Durable_Solutions)
# DS_Option5b_sudan <- targets::tar_read("DS_Option5b_sudan") %>% select(-Durable_Solutions)

ds_options_s <- 
  tribble(~label,                                ~opt,                    ~opt_nohlp,
          "Pass/fail measure",                   DS_Original_sudan, DS_Original_nohlp_sudan,
          "1: Full composite",                   DS_Option1_sudan,  DS_Option1_nohlp_sudan,
          "2: Composite at criterion level",     DS_Option2_sudan,  DS_Option2_nohlp_sudan,
          "3: Composite at sub-criterion level", DS_Option3_sudan,  DS_Option3_nohlp_sudan,
          "4: Comparison of homogenous cells",   DS_Option4_sudan,  DS_Option4_nohlp_sudan,
          "5: Classifier/regression-based",      DS_Option5_sudan,  DS_Option5_nohlp_sudan)
...
This methodological note first outlines the solutions measure as suggested by the IRIS and demonstrates the remaining methodological challenges. The main body of the note then simulates how the selection of statistical indicators and the choice of a statistical framework to combine them affect the results of a solutions measure, i.e. the magnitude of exit from IDP stocks. To conduct these simulations, three empirical datasets from Hargeisa, Nigeria and Sudan are used. The note concludes with a summary of findings to improve the solutions metrics.  
...
knitr::kable(DS, booktabs = T,
  ...
  add_header_above(c("  " = 1,
                     "Indicators from" = 1, 
                     "Of which, available in ..." = 3))%>%
  column_spec(2, width = "7.5em") %>% 
  column_spec(3:5, width = "6.5em")
```
...
Metric option 4 is based on homogeneous IDP population cells that first have to be defined before they can be assessed as a group average. For example, in El Fasher, the IDP population was grouped into subgroups based on their type of housing. This was chosen because the El Fasher questionnaire did not contain many other variables to group the IDPs into logical groups. However, in Nigeria and Hargeisa, the IDP population was split into subgroups based on core demographics, their timing of displacement and their places of displacement and origin to compare different ways of grouping the population.
...
\bigskip
  ...
  Finally, a regression-based approach requires a methodological decision how to classify an IDP household as more similar to other IDPs (and hence as still vulnerable) or more similar to host households (hence exiting the IDP stock). In this assessment a logistic classifier was fitted to determine whether a household is similar to an IDP household or a host community household. After fitting a regression, we can predict the probability that an IDP household is closer to the other IDPs or to the host community. If an individual household has a predicted probability of being similar to the host community of 0.8, we need a threshold to decide whether we exit this household from the stock as close enough to the host community or not. To do this, we optimized this threshold by using a threshold that provides the best discriminatory power between IDPs and the host community (i.e. the model best separates the IDP and host population). \textcolor{red}{XXX.} Further assessments are needed to refine this cut-off point.

\textcolor{red}{Hisham, can you write two sentences for me how to best describe how we fit the classifier. I looked at the code and I find your optimum threshold counter-intuitive? how can we optimise based on best "predictions"/ROC curve? The "best" model would be the one classifying all hosts as hosts and all IDPs as IDPs but we WANT to classify some IDPs as "hosts" - so optimization makes as more restrictive in an ex ante way, no? I do not understand why we are not just providing the picture with a simple threshold that would make it easier to understand. What's the benefit?}

If survey weights were available (in Nigeria dataset), they are considered in the simulations to ensure representativeness. 
...
\textcolor{red}{Kristine, could I ask you to read those paragraphs carefully and the let me know if the explanation for the kappa coefficient is not sufficient? I want to make this report fairly easy to read.}

```{r ira_hargeisa, fig.cap="\\label{fig:kappa-har}Inter-rater agreement between different metric options for Hargeisa", fig.width=6, fig.height=5}
metric_pairs <- read_csv("kappa.csv") %>% 
  filter(x!= "4b: Algorithmic clustering") %>% 
  filter(y!= "4b: Algorithmic clustering")  %>% 
  mutate(x = factor(x, levels = 
                      c("Pass/fail measure","1: Full composite", 
                        "2: Composite at criterion level",
                        "4: Comparison of homogenous cells (disp_type,gender,origin_district)",
                        "4: Comparison of homogenous cells (disp_type,gender,clan)",         
                        "4: Comparison of homogenous cells (disp_type,gender,depart_yr)",         
                        "4: Comparison of homogenous cells (disp_type,origin_district,clan)",
                        "4: Comparison of homogenous cells (disp_type,origin_district,depart_yr)",
                        "4: Comparison of homogenous cells (disp_type,clan,depart_yr)" ,
                        "4: Comparison of homogenous cells (gender,origin_district,clan)",
                        "4: Comparison of homogenous cells (gender,origin_district,depart_yr)",
                        "4: Comparison of homogenous cells (gender,clan,depart_yr)",
                        "4: Comparison of homogenous cells (origin_district,clan,depart_yr)",
                        "4: Comparison of homogenous cells (disp_year,arriv_year,origin)",
                        "4: Comparison of homogenous cells (disp_year,arriv_year,region)",
                        "4: Comparison of homogenous cells (disp_year,origin,region)",
                        "4: Comparison of homogenous cells (arriv_year,origin,region)"
                      #  "5: Classifier/regression-based"
                        )),
         y = factor(y, levels = 
                      c("1: Full composite", 
...
metric_pairs %>% 
  filter(dataset == "hargeisa")%>% 
  ggplot() + 
  geom_tile(aes(x, reorder(y,desc(y)), fill = kappa), color = "grey50") + 
...
To fully assess the suitability of the homogeneous cell approach, we have to assess different ways in which the IDP population can be grouped in cells. Theoretically, the expectation is that some ways of grouping the overall population will lead to larger groups and may be more lenient; other groupings could lead to smaller and/or more restrictive assessments that exit less IDPs from the stock. Two approaches were chosen to assess in the simulations how important the grouping/cell-creation is for the overall results:
...
\bigskip
\textbf{Algorithmic clustering:} Second, we employed automatic hierarchical clustering algorithms to group the IDPs into groups based on their performance on the different indicators. That means, an algorithm identifies IDP households in the data that are similiar to each other in the sense that they perform equally well on certain sub-criteria while equally bad on others. These automatic groupings yield IDP cells that are most homogeneous within the cells and most heterogeneous in comparison to other cells. This is a data-driven approach that can provide some key lessons for finding good grouping variables. The hierarchical clustering revealed:

\begin{itemize}
\item \textcolor{red}{Hisham, could I ask you to review the above paragraph and to wirte up two or three key messages that we can communicate in regard to the best selection of grouping variables?}
\end{itemize}
...
The IDP profiling in Nigeria covered a total of `r sum(data_nigeria$ID)` IDPs. Because the dataset includes survey weights, these IDPs represent an overall amount of over `r round(sum(nigeria_idps$WT)/1000,2)` thousand IDPs. On average across all simulated indicators and metrics, `r round(DS_mean_nigeria,2)`  IDPs exit the IDP stock, which corresponds to an average of `r round(DS_perc_mean_nigeria*100,2)`% of the IDPs represented by the profiling. For  an average of `r round(mean(missing_nig$missing)*100,2)`% of the IDP households represented by the profiling in Nigeria, IDP households could not be assessed due to missing data on at least one dimension. Similar to the assessment in Hargeisa, the question remains if it is acceptable that around a seventh of the IDPs remain in the stock because of missing data. 
...
```{r indicator_plots_nigeria, fig.cap="\\label{fig:indicator_nigeria}Average effect of indicator selection across metrics on IDPs exiting the stock (Nigeria)", out.width="75%"}
...
knitr::kable(NIG_metric[-1,], "latex",
             booktabs = TRUE, linesep = "",
             caption= "\\label{tab:NIG_metric}Summary of simulation results for Nigeria across all metrics (Representing over 129.41 thousand IDP households)") %>%
  kable_styling(latex_options = "hold_position", font_size=10)%>%
...
In Nigeria, the simulations iterated through 4 possible groupings based on arrival or displacement year, and region of origin or displacement. As the table below shows, most of these groupings result in very few or now IDPs exiting the stock. This may be a result of too small or too big cells of IDPs. Overall, the groupings based purely on timing and location of IDPs seems to be not optimal but further work is needed to define better grouping variables.  

\bigskip

\begin{tabular}[t]{>{\raggedright\arraybackslash}p{8em}>{\raggedright\arraybackslash}p{8em}>{\raggedright\arraybackslash}p{8em}>{\raggedleft\arraybackslash}p{6.7em}>{\raggedleft\arraybackslash}p{7em}>{\raggedright\arraybackslash}p{7em}}
\toprule
Group variable 1 & Group variable 2 & Group variable 3 & Average number of IDPs exiting the stock & Average percentage of IDPs exciting the stock & Range and mean of group size\\
\midrule
...
\bigskip
\textbf{Algorithmic clustering:} \textcolor{red}{Hisham, again, not sure what to add. }
...
\textcolor{red}{Hisam, I am not sure why we are having missing values here. Could I ask you to sent me a csv of the kappa values for Nigeria, Sudan, and Hargeisa again? I cannot calculate the scores. My computer does not have the memory for it }

```{r ira_nigeria, fig.cap="\\label{fig:kappa-nig}Inter-rater agreement between different metric options for Nigeria", fig.width=6, fig.height=5}

metric_pairs %>% 
  filter(dataset == "nigeria")%>% 
  ggplot() + 
  geom_tile(aes(x, reorder(y,desc(y)), fill = kappa), color = "grey50") + 
  scale_y_discrete(label = function(x) {str_replace(x, " \\(", "\n\\(")}) +
  scale_x_discrete(label = function(x) {str_replace(x, " \\(", "\n\\(")},
...
## Simulation results for El Fasher (JIPS 2019)
...
The IDP profiling in El Fasher (Sudan) covered a total of `r sum(data_sudan$ID, na.rm=T)` individual IDP households. As with the data from Nigeria, the results presented here are weighted by the survey weights included in the El Fasher sampling. Absolute and relative numbers should hence be compared to the number of `r round(sum(sudan_idps$WT)/1000,2)` thousand IDPs. 

When running the different metric options, the simulations all resulted in zero IDP households exiting the stock as they compare worse than the host community on the 10 sub-criteria. This finding is across all metric options and across all indicators. This is because on average across the different possible indicators, `r round(mean(missing_sud$missing)*100,2)` of the data cannot be assessed due to missingness on one of the 10 dimensions. Three of the indicators that miss the most in the data from Sudan is information on the ownership and documentation of property in the habitual residence and on access to compensation mechanisms. The HLP indicators are missing for many IDP households. If they are present and an assessment can be made, the HLP indicators are very restrictive: most IDP households do not have access to compensation or a written documentation to prove ownership. This explains why zero IDP households exit the stock across all metrics. 

In the following sections, the results of the simulations are presented when excluding the HLP dimension (sub-criterion 4.1) in a briefer summary. The section after compares more systematically how the sub-criterion  *4.1 Property restitution and compensation* affects the results for Hargeisa and Nigeria.
...
Similar to Hargeisa and Nigeria, the importance of the indicator selection differs across metrics options and the overall pattern is the same again: A composite measure at the criterion or sub-criterion is very restrictive, allowing few IDP households to exit the stock. Similarly, a comparison of homogeneous cells is restrictive and classifies few IDPs as accessing the stock. In Hargeisa, IDPs were grouped into cells based on the housing structure they live in. This resulted in some larger groups, such as 12575 IDP households that live in a Tukul made of made and only around 8 households that live in a house constructed of woods. Across those cells, very few IDP households could exit the stock. However, with other groupings - for which no suitable indicators were available in the dataset - this result could look differently. We again find that the full composite index and a regression-based approach are more lenient, identifying more exits from the IDP stock - but they are also more prone to variation depending on the indicators selected and may classify too many IDPs as no longer subjected to displacement-related vulnerabilities when in fact they are. See Figure \ref{fig:density_sudan} for the density plots of the resulting simulations. 
...
A final word on the feasibility of the different metric options in El Fasher (Sudan): To fully implement a composite measure at the criterion or sub-criterion level, there are not enough possible indicators on the household level. To implement the solutions simulation, the data from individual household members regarding their educational status, emploment and documentation had to be first aggregated, making the process not impossible but more complicated. The dataset also did not include suitable grouping variables to assess the homogeneous cells approach in different ways. 

The full composite measure and the regression-based assessment were hence the easiest to implement but they are conceptually more difficult to allow comparisons across displacement contexts and they make the task of selecting the right indicators more acute. 

## Discussion of housing, land, and property rights indicators

Amongst other results, the simulations have shown that the choice of indicators matters the most in regard to the sub-criterion *4.1 Property restitution and compensation*. This was the case in Hargeisa and Nigeria, where most indicator selections did not affect more than 1 percentage point difference in the number of IDPs exiting the stock. However, we have seen that choosing a more permissive HLP indicator can affect large percentages of the IDP stock. In the extreme case of Sudan, the property restitution indicators prevented all exits from the IDP stock.

To further investigate how the HLP indicators affect the overall simulation results in a systematic way, the figures \ref{fig:hlp_har} and \ref{fig:hlp:nig} plot the simulation densities for an assessment of all 10 sub-criteria and for an assessment based on 9 sub-criteria (excluding the HLP indicators). The blue shaded curves exclude the HLP indicators; the grey shaded curves are the original assessment.
...
To summarise, the full composite measure and the regression-based assessments are the right metric options to choose **if EGRIS wants a permissive solutions measure that exits a substantial proportion of IDPs from the overall stock**. Those metric options will yield high estimates of the IDP exit and will make it easier to "reduce" the IDP stock in reporting. Since this can be seen as a risk, it may be advisable to select relatively strict indicators to measure the sub-criterion to counteract the tendencies of those metrics options. A simple pass/fail measure (without context comparison) or a composite measure at the sub-criterion are not good options due to the high data-demands and the restrictive nature of the metrics as almost no IDP household can meet the standard. **If EGRIS wants a more restrictive solutions measure that exits few IDPs from the stock**, a possibility would be to implement a composite measure at the criterion level or to identify homogeneous IDP cells. For the composite measure, a set of clear and sufficient indicators per criterion is necessary, which was not always given in the data used in this simulation. For a cell-based approach, better grouping variables have to be identified that generate small but not to small IDP groups to assess jointly. Cells that are too big will make it hard for the IDP population to perform as well as the host community. Cells that are too small will make the assessment very crude and driven by outliers. 
...
\item Define the indicators as solutions-oriented and address missing values
...
kable(appendix, 
  ...
  column_spec(2:5, width = "15.7em") %>% 
    row_spec(c(11,12,15,29,33,38,46,58,65), hline_after = TRUE)
```
